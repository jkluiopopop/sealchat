...
说明：群晖 NAS 的 `Container Manager`、Btrfs 子卷、以及“多个外部目录分别挂载到容器”的部署方式，常会让 `/app/data`、`/app/sealchat-data`、`/app/static` 实际处于不同挂载点。若临时目录与最终存储目录分属不同挂载点，Linux 的 `rename` 会返回 `invalid cross-device link`。当前版本已兼容该情况并自动降级为复制，但仍建议将临时目录放在与最终存储目录相同的挂载点，以减少额外 I/O：

- 附件/图片：`storage.local.tempDir: ./sealchat-data/temp`
- 音频：`audio.tempDir: ./static/audio-temp`
...
SealChat 支持将附件/图片、音频，以及平台字体原文件/分片资源存入 S3（或兼容协议的对象存储，如 MinIO、腾讯 COS 等）。相关配置在 `config.yaml` 的 `storage` 段，建议直接参考并复制 `config.yaml.example` / `config.docker.yaml.example` 中的示例，再按实际替换。
...
- 本地目录（当对应类型走本地时生效）
  ...
  - `storage.local.fontDir`：平台字体原文件与字体分片目录
  - `storage.local.tempDir`：临时目录
- NAS/Btrfs 建议：若使用 Docker 多挂载目录，最好让 `storage.local.tempDir` 与 `storage.local.uploadDir` 处于同一挂载点；否则虽然服务会自动回退为复制，但上传时会增加一次额外 I/O。
- 音频导入目录（用于素材库“读取数据目录”功能）
  - `audio.importDir`：扫描导入目录，默认 `${audio.storageDir}/import`
  - `audio.tempDir`：建议与 `audio.storageDir` 保持同一挂载点（例如 `./static/audio-temp`）
