...
除 HTML 内容本身外，模板库模板还支持一个宿主侧配置项：

- 默认角色徽章模板：使用 `{属性名}` 占位，例如 `HP{生命值} SAN{理智}`

该配置项不注入 iframe，不属于 `window.sealchat` API。它只在“用户显式选择模板库模板”时由宿主处理：

- 若当前世界已配置服务端默认徽章模板，宿主不会改动本地设置
- 若当前世界未配置服务端默认徽章模板，且模板填写了“默认角色徽章模板”，宿主会将当前世界本地徽章模板更新为该值
- 切换为自定义模板、编辑模板 HTML、模板内部调用 `sealchat.updateAttrs(...)` 都不会触发此同步

#### 2.0.1 使用方式

这个配置项**不是写在 HTML 模板里的代码**，也**不是**通过 `window.sealchat` 在运行时设置。

正确做法：

1. 在模板管理器中创建或编辑模板
2. 在“默认角色徽章模板”输入框里填写模板字符串
3. 保存模板
4. 用户在人物卡窗口中**显式选择该模板库模板**时，宿主再决定是否同步当前世界本地徽章模板

错误做法：

- 不要在 HTML 里写 `<meta>`、`data-*`、注释约定来声明这个值
- 不要尝试调用不存在的 API，例如 `sealchat.setBadgeTemplate(...)`
- 不要指望模板 iframe 在加载时自动修改徽章模板

#### 2.0.2 写法规则

填写内容时，直接写最终想显示的徽章模板文本，使用 `{属性名}` 占位。

示例：

- `HP{生命值} SAN{理智} MP{魔法值}`
- `力量{力量} 敏捷{敏捷} 体质{体质}`
- `侦查{侦查} 聆听{聆听} 闪避{闪避}`

说明：

- `{生命值}` 表示读取人物卡 attrs 中键名为 `生命值` 的值
- 占位符名称必须和人物卡属性键完全一致
- 这里只控制“角色徽章显示文本”，不影响 iframe 中人物卡 HTML 的渲染逻辑
- 留空表示该模板不干预当前世界本地徽章模板

#### 2.0.3 与 HTML 模板代码的关系

可以把它理解成两套独立配置：

- HTML 模板：决定人物卡窗口里怎么渲染、怎么响应点击、怎么发 `ROLL_DICE` / `UPDATE_ATTRS`
- 默认角色徽章模板：决定用户选择这个模板库模板时，是否顺手更新当前世界本地徽章模板

也就是说，下面这种 HTML 模板本身**不会**声明默认角色徽章模板：
...
如果你希望这个模板配套一个推荐徽章模板，需要在模板管理器的“默认角色徽章模板”字段里单独填写，例如：
